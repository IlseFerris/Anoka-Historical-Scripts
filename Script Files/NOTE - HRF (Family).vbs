...
If datediff("d", "08/06/2013", now) < 4 then MsgBox "This script has been updated as of 08/06/2013! Here's what's new:" & chr(13) & chr(13) & "There's now a ''FIAT reasons'' section at the end of the case note. Make sure you fill in any reasons you FIATed this case. This is required per the PCs."
...
BeginDialog HRF_dialog, 0, 0, 451, 245, "HRF dialog"
  ...
  Text 5, 5, 190, 15, "Note: you can use a semicolon '';'' to force a new line in a section of this case note."
...
If STAT_check <> "STAT" then call script_end_procedure("Can't get in to STAT. This case may be in background. Wait a few seconds and try again. If the case is not in background email Veronica Cary the case number and footer month.")
...
Do
  Do
    ...
    If earned_income = "" or actions_taken = "" or HRF_datestamp = "" or worker_signature = "" then MsgBox "You need to fill in the datestamp, earned income, and actions taken sections, as well as sign your case note. Check these items after pressing ''OK''."
  Loop until earned_income <> "" and actions_taken <> "" and HRF_datestamp <> "" and worker_signature <> ""
