...
name_of_script = "NOTE - REVW-MONT closures"
start_time = timer
...
BeginDialog REVW_case_note_dialog, 0, 0, 226, 115, "REVW case noter"
  EditBox 140, 15, 80, 15, worker_signature
  EditBox 160, 40, 45, 15, x102_number
  CheckBox 15, 80, 120, 10, "REPT/MONT? (HRFs)", MONT_check
  CheckBox 15, 95, 120, 10, "REPT/REVW? (CSRs and ARs)", REVW_check
  ButtonGroup ButtonPressed
    OkButton 165, 70, 50, 15
    CancelButton 165, 95, 50, 15
  Text 5, 5, 130, 25, "This script will case note all of your renewals that are closing/incomplete. You'll need to sign your case notes:"
  Text 5, 45, 150, 10, "Enter the last three digits of your x102# here:"
  GroupBox 5, 65, 150, 45, "Case note closing/incomplete cases from:"
EndDialog
...
Do
  Do
    Dialog REVW_case_note_dialog
    If ButtonPressed = 0 then StopScript 'Cancel button
    If x102_number <> "" then x102_number = ucase(x102_number)
    If len(x102_number) <> 3 then MsgBox "You must enter the last three digits of your x102# (and just the last three digits)."
...
If revw_check = 1 then 
  ...
  EMReadScreen default_x102_number, 3, 21, 10
  If x102_number <> default_x102_number then
...
'THIS IS FOR TESTING ONLY AND MUST BE REMOVED<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
'test_box = Msgbox ("Cash: " & cash_review_status & chr(13) & "WB: " & WB_review_status & chr(13) & "FS: " & FS_review_status & chr(13) & "HC: " & HC_review_status & chr(13) & "Last day to turn in docs: " & last_day_to_turn_in_docs & chr(13) & "Intake date: " & intake_date, 1)
'If test_box = 2 then stopscript
...
If mont_check = 1 then 
  ...
  'Checking the current X102 number. If it's not the selected one it will enter the selected one.
  EMReadScreen default_x102_number, 3, 21, 10
  If x102_number <> default_x102_number then
    EMWriteScreen x102_number, 21, 10
    transmit
...
'<<<<<<<  THE FOLLOWING SECTION DETERMINES INTAKE/ONGOING LOGIC. HEATHER ASKED ME TO REMOVE IT FOR NOW AS
'<<<<<<<    IT DOESN'T ALWAYS WORK CORRECTLY. CASES CAN STAY ON MONT FOR A FEW MONTHS, AND THE INFO ABOUT
'<<<<<<<    INTAKE/ONGOING SHOULDN'T CHANGE. PER HEATHER, WE CAN WAIT ON INTRODUCING MORE INFO UNTIL IT'S
'<<<<<<<    REQUESTED BY OTHER WORKERS.

'    If cash_review_code = "N" then cash_review_status = "HRF not provided."
'    If cash_review_code = "I" then cash_review_status = "there is an incomplete HRF. See previous case notes for details on what's needed."
'    If GRH_review_code = "N" then GRH_review_status = "HRF not provided."
'    If GRH_review_code = "I" then GRH_review_status = "there is an incomplete HRF. See previous case notes for details on what's needed."
'    If FS_review_code = "N" then FS_review_status = "HRF not provided."
'    If FS_review_code = "I" then FS_review_status = "there is an incomplete HRF. See previous case notes for details on what's needed."
'    If HC_review_code = "N" then HC_review_status = "HRF not provided."
'    If HC_review_code = "I" then HC_review_status = "there is an incomplete HRF. See previous case notes for details on what's needed."
  
'  '---------------THIS SECTION FIGURES OUT WHEN PROGRAMS CAN TURN IN NEW RENEWALS AND WHEN THEY BECOME INTAKES AGAIN
'    If cash_review_status <> "" or GRH_review_status <> "" or FS_review_status <> "" or HC_review_status <> "" then
'      EMReadScreen first_of_working_month, 5, 20, 55
'      first_of_working_month = cdate(replace(first_of_working_month, " ", "/01/"))
'      last_day_to_turn_in_docs = dateadd("d", -1, (dateadd("m", 1, first_of_working_month)))
'      intake_date = dateadd("m", 1, first_of_working_month)
...
'THIS IS FOR TESTING ONLY AND MUST BE REMOVED<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
'test_box = Msgbox ("Cash: " & cash_review_status & chr(13) & "GRH: " & GRH_review_status & chr(13) & "FS: " & FS_review_status & chr(13) & "HC: " & HC_review_status & chr(13) & "Last day to turn in docs: " & last_day_to_turn_in_docs & chr(13) & "Intake date: " & intake_date, 1)
'If test_box = 2 then stopscript
