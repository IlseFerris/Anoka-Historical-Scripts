...
'<<<<THESE WERE ADDED IN A BULK LOAD AND NEED TO BE WORKED INTO THE REST OF THE SCRIPT!!!
...
EMReadScreen second_line_check, 1, row + 1, 5
...
If stat_check <> "STAT" then
  MsgBox "This case couldn't get to stat. MAXIS may have slowed down or be in background. Try again in a few seconds. If this continues to happen and MAXIS is up, send the case number to the script administrator."
  Stopscript
End if
...
If HH_memb_check = "REFERENCE NUMBER IS INVALID    " then 
  MsgBox "That member number is invalid. The script will now stop. Try again from DAIL. If this keeps happening, send the case number to the script administrator."
  stopscript
End if
If HH_memb_check = "MEMBER " & HH_memb & " IS NOT IN THE HOUSEHO" then 
  MsgBox "That member number is invalid. The script will now stop. Try again from DAIL. If this keeps happening, send the case number to the script administrator."
  stopscript
End if
If HH_memb_check = "OCCURRENCE NUMBER IS INVALID   " then 
  MsgBox "That member number is invalid. The script will now stop. Try again from DAIL. If this keeps happening, send the case number to the script administrator."
  stopscript
End if
...
BeginDialog job_known, 0, 0, 191, 76, "Job known?"
  ...
  ButtonGroup ButtonPressed_job_known
...
Dialog job_known
If ButtonPressed_job_known = 0 then stopscript
...
If jobs_check <> "JOBS" or jobs_memb_check <> HH_memb then
  MsgBox "You appear to have navigated away from the JOBS panel for member " & HH_memb & ". The script will now stop. Try again from your DAIL. If this keeps happening, send the case number and a description of what happened to the script administrator."
  StopScript
End if
...
EMWriteScreen "1", 12, 67
EMWriteScreen "1", 18, 72
...
EMSendKey worker_sig + ", using automated script." + "<PF3>"
EMWaitReady 1, 1
EMSendKey "<PF3>"
EMWaitReady 1, 1
...
EMSetCursor 9, 3
EMSendKey "Verification of new employer (via CS message) should have returned by now. If not received and processed, take appropriate action. (TIKL auto-generated from script)." + "<enter>"
EMWaitReady 1, 1
EMSendKey "<PF3>"
