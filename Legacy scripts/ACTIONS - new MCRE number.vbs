'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "new MCRE number"
start_time = timer
...
EMReadScreen MMIS_A_check, 7, 15, 15 'This should be row 15 (the middle of the three coordinates!) for production and row 16 for training.
IF MMIS_A_check = "RUNNING" then EMSendKey "10" + "<enter>" 'This should be 10 for production and 11 for training.
...
IF MMIS_A_check <> "RUNNING" then EMSendKey "<attn>"
EMWaitReady 1, 0
IF MMIS_A_check <> "RUNNING" then EMReadScreen MMIS_B_check, 7, 15, 15 'This should be row 15 for production and row 16 for training.
...
IF MMIS_A_check <> "RUNNING" and MMIS_B_check = "RUNNING" then EMSendkey "10" + "<enter>" 'This should be 10 for production and 11 for training.
...
'Now we're in the next screen, and the worker has to enter a PMI number.
  BeginDialog PMI_dialog, 0, 0, 136, 87, "PMI dialog"
    EditBox 65, 0, 65, 15, PMI_number
    ButtonGroup PMI_dialog_ButtonPressed
      OkButton 15, 65, 50, 15
      CancelButton 70, 65, 50, 15
    Text 5, 5, 55, 10, "Enter your PMI:"
    Text 5, 20, 130, 40, "DO NOT press OK until you've checked to see if this client has a MCRE number already. If you have not checked, press cancel and look the client up in MMIS. Otherwise, press OK."
  EndDialog
